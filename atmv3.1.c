...
while(user_state != EXIT){  // we check if user is logged in or out so that we can call the correct function if the return value is EXIT, we break the program
...
int handleSubMenu() // this is the menu if you are logged in, you make your operations here and where the main program lives.
{
    ...
    if (fp == NULL) {                   // to prevent wrong balance output(operations do their job successfully but it wasn't shown
                                        // at this menu)
...
void registration() { // the registration function, it creates a file named has fname(declared as global variable)
    ...
    for (int i = 0; i < num_users; i++) {
        if (strcmp(l.username, existing_users[i].username) == 0) { // we compare here if user is already exist and declare isUserExist=1 and
    ...
    else {              // after succesfull signing up- we write their data to "fname" file and ask them deposit some money so they can operate, or they can do it
                        // at the subMenu
        int deposit;
                ...
                // sign up or close the program.
{
    ...
    if(log == NULL){ // checks if the file exists and returns an error message that wants them to sign up.
...
void withdraw(){
	...
	scanf("%d",&cost);
    if (cost > wdraw.balance){      // checks for the conditions to prevent wrong user input
        printf("Girdiginiz deger bakiyenizden fazla olamaz.!\n");
        fclose(fp);
    }
    else if (cost > 200000){        // checks for the conditions to prevent wrong user input
...
void deposit(){	    // similar to wdraw function but operation is different
	...
	printf("Lutfen yatirmak istediginiz miktari giriniz: ");
	scanf("%d",&cost);
    if (cost < 0){                                         // checks for the conditions to prevent wrong user input
...
void transfer(){    // same as wdraw function, just the change of names
...
int currentBalance(){   // this is for the synchronising the balance data so that we can see it real time
